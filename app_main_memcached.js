...
let memcached = require('memcached');  // https://dev.to/pankajkumar/how-to-implement-memcached-in-nodejs-application-105a

// let memC = new memcached();

let memC = new memcached('localhost:11211');

// let memC = new memcached('127.0.0.1:11211');

// let memC = new memcached('memcached-tool /opt/memcached/run/teache13/memcached-1.sock');
// let memC = new memcached('/opt/memcached/run/teache13/memcached-1.sock:0');

// let memC = new memcached('memcached-tool /opt/memcached/run/teache13/memcached-1.sock:0');

// memcached-tool /opt/memcached/run/teache13/memcached-1.sock stats
// let memC = new memcached('/opt/memcached/run/teache13/memcached-1.sock', 0);
// let memC = new memcached('memcached-tool /opt/memcached/run/teache13/memcached-1.sock:0');
// let memC = new memcached('/opt/memcached/run/teache13/memcached-1.sock:0');
// let memC = new memcached('memcached-tool /opt/memcached/run/teache13@az1-ts102/memcached-1.sock');
// let memC = new memcached('memcached-tool /opt/memcached/run/teache13@az1-ts102/memcached-1.sock:0');

// telnet memcached-tool /opt/memcached/run/teache13/memcached-1.sock 0
...
// memC.connect( 'localhost:11211', function( err, conn ){ if( err ) { console.log( conn.server,'error while memcached connection!!'); } });
// memC.connect( '/opt/memcached/run/username/memcached-1.sock:0', function( err, conn ){ if( err ) { console.log( conn.server,'error while memcached connection!!'); } });
...
io.on('connection', (socket) => { 
    ...
    socket.on('check_type_of_gCode', (gCode, socketResp) => {
        memC.getMulti([gCode+'_game_status',gCode+'_teacher_details'], (err,data) => { 
            if (data[gCode+'_game_status']) { let gState = data[gCode+'_game_status'].split('>>')[0];
                if (gState != 'deleted' || gState != 'finished') {               // [ state, timeStamp, round, maxGroupSize, currentGame, total_rnds ]
                    socketResp(data[gCode+'_teacher_details']);
                }
    ...
    socket.on('link_user_to_teacher_via_gCode', (params, socketResp) => { let newArrayOfStudents;     let linkedTeachers;
        ...
        memC.get(params.gCode+'_teacher_details', (err, teacherDetails) => { let teacher_details = teacherDetails.split('>>');        // [teacherID, teacherName, classPW, arrayOfStudents]
            ...
            if (user_and_teacher_NOT_linked) {
                pool_users_db.query("SELECT studentFullName FROM userID_"+params.ref+"_activities WHERE activity='history'", params, (error, results2, fields)=>{
                    ...
                    memC.replace(pass_1.gCode+'_teacher_details', teacher_details, 7200, (err)=>{
    ...
    socket.on('link_newUser_to_parentTutor', (params, socketResp) => { // { "userRef":userRef,"userName":userName,"teacherRef":idKey,"teacherName":user, "arrayOfStudents":arrayOfStudents }
        ...
        // let newArrayOfStudents;     let linkedTeachers;

        // memC.get(params.gCode+'_teacher_details', (err, teacherDetails) => { let teacher_details = teacherDetails.split('>>');        // [teacherID, teacherName, classPW, arrayOfStudents]
        
        // params.teacherRef = teacher_details[0];              params.teacherName = teacher_details[1];
        // params.arrayOfStudents = teacher_details[3];        // params.classPW = teacher_details[2];
        ...
        pool_teachers_db.query("UPDATE teacherID_"+params.teacherRef+" SET arrayOfStudents='"+newArrayOfStudents+"' WHERE ref='"+params.teacherRef+"'", params, (error, results, fields)=>{ 
            ...
            // if (pass_1.currentLinkedTeachers == 'none') { linkedTeachers = pass_1.teacherRef +"_"+ pass_1.teacherName }
            
            // else { linkedTeachers = pass_1.currentLinkedTeachers + "," + pass_1.teacherRef +"_"+ pass_1.teacherName }
    ...
    function retrieve_and_check_memC_data(gCode,memCfunction) {
        
        memC.getMulti([gCode+'_game_status',gCode+'_linked_users',gCode+'_user_results',gCode+'_user_ranking',gCode+'_teacher_details'], (err,data) => {

            if (err || !data || data == undefined || !data[gCode+'_game_status'] || data[gCode+'_game_status'] == undefined || !data[gCode+'_linked_users'] || data[gCode+'_linked_users'] == undefined || data[gCode+'_user_results'] == undefined || data[gCode+'_user_ranking'] == undefined || !data[gCode+'_teacher_details'] || data[gCode+'_teacher_details'] == undefined) {

                pool_teachers_db.query("SELECT * FROM "+gCode+" WHERE gCode='"+gCode+"'", gCode, (error, results, fields)=>{ // socketResp(results[0].game_status);

                    memC.set(gCode+'_game_status', results[0].game_status, 7200, (err) => { // set for 2 hr = 7200 seconds
                        memC.set(gCode+'_linked_users', results[0].linked_users, 7200, (err) => {
                            memC.set(gCode+'_user_results', results[0].user_results, 7200, (err) => {  
                                memC.set(gCode+'_user_ranking', results[0].user_ranking, 7200, (err) => { 
                                    memC.set(gCode+'_teacher_details', results[0].teacher_details, 7200, (err) => {
            ...
            else { memCfunction(data); }
        });
    }

    socket.on('check_num_users_linked_to_game', (gameCode, socketResp) => {
        retrieve_and_check_memC_data(gameCode, (data) => { 
            let numOfUser = 0;
            numOfUser = data[gameCode+'_linked_users'].split('|').length - 2;
            socketResp(data[gameCode+'_game_status']+'|>|'+numOfUser);                    // state>>timeStamp>>round>>maxGroupSize>>currentGame
        });
    });
    ...
    socket.on('get_guest_gameCode', (params,socketResp) => { 
        ...
        let getGameCode = function(params) { 
            ...
            memC.get(gameCode+'_teacher_details', (err, results) => { 
                if (results || gameCode == 'arse') { getGameCode(params); } // try again for a unique gameCode
                else {
                    let pass = params;
                    let game_status = "not_started_yet>>"+Date.now().toString()+">>0>>"+pass.maxGroupSize+">>"+pass.currentGame;

                    memC.set(pass.gameCode+'_game_status', game_status, 7200, (err) => { // set for 2 hr = 7200 seconds
                        memC.set(pass.gameCode+'_linked_users', '|', 7200, (err) => {
                            memC.set(pass.gameCode+'_user_results', '', 7200, (err) => {  
                                memC.set(pass.gameCode+'_user_ranking', '', 7200, (err) => { 
                                    memC.set(pass.gameCode+'_teacher_details', 'nonMember>>guest', 7200, (err) => { 
                                        memC.set(pass.gameCode+'_last_reBoot', 'none', 7200, (err) => { 
    ...
    socket.on('start_game_sequence', (params) => { //params = [ "state", "timeStamp", "round", "maxGroupSize", "currentGame", "total_rnds"]
        ...
        memC.replace(params.gameCode+'_game_status', game_status, 7200, (err) => { // start_clicked
    ...
    let get_ready_to_START_new_round = function(gameCode) {  // ################### GAME LOOP #################################################################
        ...
        retrieve_and_check_memC_data(gameCode, (data) => { 
            ...
            let game_status = data[gameCode+'_game_status'].split('>>');                           // [ state, timeStamp, round, maxGroupSize, currentGame, total_rnds ]
            ...
            memC.replace(gameCode+'_game_status', game_status_wait, 7200, (err) => {
                    ...
                    setTimeout( () => { io.in(gameCode).emit(gameCode+'_countDown', '1');  // socket.emit(gameCode+'_countDown', '1');
                        ...
                        setTimeout( () => {
                            ...
                            retrieve_and_check_memC_data(gameCode, (data) => { 
                                ...
                                memC.replace(gameCode+'_game_status', game_status_running, 7200, (err) => {
                            ...
                            setTimeout( () => {
                                retrieve_and_check_memC_data(gameCode, (data) => { 
                                    ...
                                    memC.replace(gameCode+'_game_status', game_status_60s_complete, 7200, (err) => {
                                ...
                                setTimeout( () => {
                                    retrieve_and_check_memC_data(gameCode, (data) => { 
                                        io.in(gameCode).emit(gameCode+'_retreive_all_user_round_data', data[gameCode+'_user_results']);
                                    });
                                        ...
                                        retrieve_and_check_memC_data(gameCode, (data) => {
                                            ...
                                            if (game_status[2] == game_status[5]) { // therefore game has "finished" all rounds
                                                ...
                                                if (gameCode[0] == 'a' || gameCode[0] == 'b') { 
                                                    memC.replace(gameCode+'_game_status', game_status_finished, 7200, (err) => {
                                                    ...
                                                    let last_game_results = data[gameCode+'_user_results'];
                                                    let last_game_ranking = data[gameCode+'_user_ranking'];
                                                    let teacherDetails = data[gameCode+'_teacher_details'];
                                                ...
                                                let game_status_waiting = data[gameCode+'_game_status'].replace('>>paused','>>waiting');
                                                memC.replace(gameCode+'_game_status', game_status_waiting, 7200, (err) => { 
                                                ...
                                                last_game_results = data[gameCode+'_user_results'];
                                                last_game_ranking = data[gameCode+'_user_ranking'];
                                                teacherID = data[gameCode+'_teacher_details'].split('>>')[0];
    ...
    socket.on('entering_game_after_start', (gameCode) => {
        retrieve_and_check_memC_data(gameCode, (data) => { 
            if (data[gameCode+'_game_status'].split('>>')[0] == 'running') {                      // [ state, timeStamp, round, maxGroupSize, currentGame, total_rnds ]
                socket.emit(gameCode+'_get_ready_to_START', data[gameCode+'_game_status']);
    ...
    socket.on('game_paused_by_teacher', (gameCode,socketResp) => { 
        retrieve_and_check_memC_data(gameCode, (data) => { 
            let game_status_paused = data[gameCode+'_game_status']+">>paused";  
            memC.replace(gameCode+'_game_status', game_status_paused, 7200, (err) => { 
    ...
    socket.on('game_reStart_from_paused', (gameCode,socketResp) => { 
        retrieve_and_check_memC_data(gameCode, (data) => { 
            ...
            let game_status = data[gameCode+'_game_status'].split('>>');        // socketResp(game_status);

            if (game_status[6] == 'paused') {
                memC.replace(gameCode+'_game_status', data[gameCode+'_game_status'].replace('>>paused',''), 7200, (err) => { 
                    socketResp('gameCode_'+gameCode+'_turn_paused_OFF');
                });
            }
            
            if (game_status[6] == 'waiting') {
                memC.replace(gameCode+'_game_status', data[gameCode+'_game_status'].replace('>>waiting',''), 7200, (err) => { 
                    get_ready_to_START_new_round(gameCode);
    ...
    socket.on('reBoot_at_round_end', (gCode_Rnd) => { // reboot if no response after 63 sec game time
        ...
        memC.get(gCode+'_game_status', (err, game_status) => {
            if (game_status.split('>>').length == 6) {                          // check if game has been 'paused' before reBooting the start of the next round
                ...
                memC.get(gCode+'_last_reBoot', (err, last_reBoot) => { 
                    ...
                    if (reBoot_atRoundEnd) {    
                        ...
                        memC.replace(gCode+'_last_reBoot', gRnd+'>reBoot_at_round_end', 7200, (err) => {
    ...
    socket.on('reBoot_to_start_game_after_loaded', (gCode_Rnd) => { // reboot if no response after 63 sec game time
        ...
        memC.get(gCode+'_last_reBoot', (err, last_reBoot) => {
            ...
            if (reBoot_toStartGameAfterLoaded) {
                ...
                memC.replace(gCode+'_last_reBoot', gRnd+'>reBoot_to_start_game_after_loaded', 7200, (err) => {    
    
                    retrieve_and_check_memC_data(gCode, (data) => { 
            
                        let game_status = data[gCode+'_game_status'].split('>>');       // [ state, timeStamp, round, maxGroupSize, currentGame, total_rnds ]
                        let game_status_running;
                        if (game_status[0] == 'deleted') { game_status_running = "deleted>>endGame"; }
                        else {
                            if (game_status.length == 7) {
                        ...
                        memC.replace(gCode+'_game_status', game_status_running, 7200, (err) => {
    ...
    socket.on('reBoot_after_62sec_of_running', (gCode_Rnd) => { // reboot if no response after 63 sec game time
        ...
        memC.get(gCode+'_last_reBoot', (err, last_reBoot) => {
            ...
            if (reBoot_after62secOfRunning) {
                ...
                memC.replace(gCode+'_last_reBoot', gRnd+'>reBoot_after_62sec_of_running', 7200, (err) => { 
                    ...
                    retrieve_and_check_memC_data(gCode, (data) => { 
                        ...
                        memC.replace(gCode+'_game_status', game_status_60s_complete, 7200, (err) => {
                    ...
                    setTimeout( () => {
                        retrieve_and_check_memC_data(gCode, (data) => { 
                            io.in(gCode).emit(gCode+'_retreive_all_user_round_data', data[gCode+'_user_results']);
                        });
                            ...
                            retrieve_and_check_memC_data(gCode, (data) => {
                                ...
                                if (game_status[2] == game_status[5]) { // therefore game has "finished" all rounds
                                    ...
                                    if (gCode[0] == 'a' || gCode[0] == 'b') { 
                                        memC.replace(gCode+'_game_status', game_status_finished, 7200, (err) => {
                                        ...
                                        let last_game_results = data[gCode+'_user_results'];
                                        let last_game_ranking = data[gCode+'_user_ranking'];
                                        let teacherDetails = data[gCode+'_teacher_details'];
                                        let teacherID = teacherDetails.split('>>')[0];
                                    ...
                                    let game_status_waiting = data[gCode+'_game_status'].replace('>>paused','>>waiting');
                                    memC.replace(gCode+'_game_status', game_status_waiting, 7200, (err) => { 
                                    ...
                                    last_game_results = data[gCode+'_user_results'];
                                    last_game_ranking = data[gCode+'_user_ranking'];
                                    teacherID = data[gCode+'_teacher_details'].split('>>')[0];
    ...
    //##############################################################################################
    /////// ### following socket connection are from the teacher "assign student_tasks" page //////////////
    ...
    socket.on('join_teacher_gameRoom', (gameCode,socketResp) => { 
        ...
        memC.getMulti([gameCode+'_game_status',gameCode+'_linked_users'], (err,data) => {
            if (data[gameCode+'_game_status'] && data[gameCode+'_linked_users']) {
                socketResp( data[gameCode+'_game_status']+'>>>>'+data[gameCode+'_linked_users'] );
            }
        });
    });
    ...
    socket.on('update_user_gameScore', (gameRound_user_results) => {            // gameCode = userRef>>score>>respTime>>round>>user>>groupColour|
        ...
        memC.append(gameCode+'_user_results', userResults, (err) => {
        });
    });

    socket.on('update_user_rankings', (gameRound_user_ranking) => { gCode_gRndUserRankings =  gameRound_user_ranking.split('=');
        memC.append(gCode_gRndUserRankings[0]+'_user_ranking', gCode_gRndUserRankings[1], (err) => { });
    });
...
app.get('/teacher/:ref', (req, res) => { let ref = req.params.ref;
    ...
    if (ref == 'nonMember') { res.render('teacher/teacher_menu_230328', {ref: 'nonMember', user: 'guest', game: 'none', maxStudentGroup: '4' }); }
    else {
        pool_teachers_db.query("SELECT firstName, gameCode, teacherName, maxStudentGroup FROM teacherID_"+ref+" WHERE ref='"+ref+"' LIMIT 1", ref, (error, results, fields)=>{
            if (error || !results || results == undefined) { }
                ...
                if (gameCode[0] == 'c') {
                    ...
                    res.render('teacher/teacher_menu_230328', { ref:ref, user:results[0].firstName, game:gameCode, teacherName:results[0].teacherName, maxStudentGroup:results[0].maxStudentGroup });
                }
            }
        });
    }
    ...
    if (req.params.ref == 'nonMember') { res.render('submit_game_code', { ref:'nonMember', user:'guest', teacherLinks:'none', alert_gameCode:'' }); }
        ...
        pool_users_db.query("SELECT activityNotes, gameCode FROM userID_"+req.params.ref+"_activities WHERE activity='teacherCodes' LIMIT 1", req.params, (error, results, fields)=>{ 
            ...
            if (results[0].gameCode == 'none') { res.render('submit_game_code', { ref:params.ref, user:params.user, teacherLinks:results[0].activityNotes, alert_gameCode:'' }); }
                ...
                memC.get(results[0].gameCode+'_game_status', (err, gameStatus) => { gState = gameStatus.split('>>')[0];
                    if (gState != 'deleted' || gState != 'finished') {
                        // then get memC of gameCode_student_groups and send through as well.  if it equals 'none' then either no student groupings.  if it doesn't exist then it is a single game
                        // in play_game the student worls out which group it is in, or none
                        // memC.get(results[0].gameCode+'_linked_users', (err, linked_users) => {
                            res.render('teacher/play_game_230328', {gameCode:results[0].gameCode, ref:params.ref, user:params.user});
                        // });
                    }
    ...
    pool_teachers_db.query("SELECT * FROM teacherID_"+req.params.ref+" WHERE ref='"+req.params.ref+"' LIMIT 1", req.params, (error, results, fields)=>{
        ...
        if (results[0].gameCode != 'none') {
            ...
            let getGameCode = function(thisTeacher) { 
                ...
                // memC.touch(gameCode+'_teacher_details', 2, (err) => { res.send(err); //});
                
                // res.send('in else of /teacher/get_game_code/ JUST before memC.get');

                memC.get(gameCode+'_teacher_details', (err, isResults) => {  // res.send(isResults);
                    if (isResults || gameCode=='cunt' || gameCode=='fuck' || gameCode=='dick' || gameCode=='slut' || gameCode=='fukk' || gameCode=='shit' || gameCode=='kunt' || gameCode=='piss' || gameCode=='pooo' ) { 
                        ...
                        memC.set(gCode+'_teacher_details', teacher_details, 7200,                   (err, teacherDetailsState) => { // res.send(err);
                            memC.set(gCode+'_linked_users', '|', 7200,                              (err, linkedState)      => {
                                memC.set(gCode+'_user_results', '', 7200,                           (err, userResultsState) => {
                                    memC.set(gCode+'_user_ranking', '', 7200,                       (err, userRankingState) => {
                                        memC.set(gCode+'_game_status', game_status, 7200,           (err, gStatusState)     => {
                                            memC.set(gCode+'_last_reBoot', 'none', 7200,    (err) => { 
    ...
    pool_teachers_db.query("SELECT * FROM teacherID_"+req.params.ref+" WHERE ref='"+req.params.ref+"' LIMIT 1", req.params, (error, results, fields)=>{
        ...
        if (results[0].gameCode != 'none') {
            ...
            let getGameCode = function(thisTeacher) { 
                ...
                memC.get(gameCode+'_teacher_details', (err, isResults) => { 
                    if (isResults || gameCode=='cunt' || gameCode=='fuck' || gameCode=='dick' || gameCode=='slut' || gameCode=='fukk' || gameCode=='shit' || gameCode=='kunt' || gameCode=='piss' || gameCode=='pooo' ) { 
                        getGameCode(thisTeacher);  // try again for a unique gameCode
                    }
                    else {
                        ...
                        memC.set(gCode+'_teacher_details', teacher_details, 7200,                   (err, teacherDetailsState) => { 
                            memC.set(gCode+'_linked_users', '|', 7200,                              (err, linkedState)      => {
                                memC.set(gCode+'_user_results', '', 7200,                           (err, userResultsState) => {
                                    memC.set(gCode+'_user_ranking', '', 7200,                       (err, userRankingState) => {
                                        memC.set(gCode+'_game_status', game_status, 7200,           (err, gStatusState)     => {
        ...
        if ( results.length === 0 ) { 
            ...
            pool_teachers_db.query("UPDATE teacherID_"+params.ref+" SET gameCode='none' WHERE ref='"+params.ref+"'", params, (error, results, fields)=>{
                ...
                pool_teachers_db.query("DELETE FROM aaaa_List_of_gameCodes WHERE gameCode='"+pass.gameCode+"'", pass, (error, results, fields)=>{
                    ...
                    memC.get(pass_1.gameCode+'_linked_users', (err, linked_users) => {
                        ...
                        memC.replace(pass_1.gameCode+'_game_status', "deleted>>endGame", 7200, (err) => {
    ...
    pool_teachers_db.query("SELECT gameCode, teacherID FROM aaaa_List_of_gameCodes WHERE gameCode='"+params.gameCode+"'", params, (err, results, fields)=>{
        if (err) { res.render('submit_game_code', { ref: params.ref, user: params.user, teacherLinks: params.currentLinkedTeachers, alert_gameCode: 'Error. Please refresh page.' }); }
            ...
            pool_users_db.query("UPDATE userID_"+params.ref+"_activities SET gameCode='"+params.gameCode+"' WHERE activity='teacherCodes'", params, (error, results, fields)=>{ 
                ...
                memC.get(params_2.gameCode+'_linked_users', (err,linked_users) => { if (err) {  } /// call db
                    ...
                    if (!linked_users) { 
                        ...
                        if (user_NOT_linked) {
                            ...
                            memC.replace(params_2.gameCode+'_linked_users', linkedUsersAppended, 7200, (err) => { // start_clicked    
                                ...
                                pool_teachers_db.query("UPDATE "+params_2.gameCode+" SET linked_users='"+linkedUsersAppended+"' WHERE gCode='"+params_2.gameCode+"'", params_2, (err, results, fields)=>{
                                    res.render('teacher/play_game_230328', {gameCode:params_2.gameCode,ref:params_2.ref,user:params_2.user});
                                });
                            });                            
                        }
                        else  { res.render('teacher/play_game_230328', {gameCode:params_2.gameCode,ref:params_2.ref,user:params_2.user}); }
                    }        
                });
            });
        }
...
app.post('/link_casual_user_to_gameCode', (req, res) => { // req.body = .ref, .user, .currentLinkedTeachers, .gameCode
    ...
    memC.get(req.body.gameCode+'_teacher_details', (err, results) => { // let teacher_details = teacherDetails.split('>>');
        ...
        let getUniqueRef = function(params) {
            memC.get(params.gameCode+'_linked_users', (err, linked_users) => {
                ...
                if (isUniqueRef) {
                    ...
                    memC.append(params.gameCode+'_linked_users', params.ref+'>'+params.user+'|', (err) => {  // this addes the user to the 'gameCode_linked_users'
                        res.render('teacher/play_game_230328', {gameCode:params.gameCode,ref:params.ref,user:params.user});
                    });
                }
                else {
            ...
            pool_teachers_db.query("SELECT * FROM teacherID_"+results[0].idKey+" WHERE ref='"+results[0].idKey+"'", (err, thisTeach, fields)=>{
                if (err) { res.render('teacher/teacher_log_in', { ref: 'nonMember', user: 'guest', game: 'none', alert_username: 'Not a valid username', alert_password: '' }); }
                    ...
                    if (results[0].access == 'schoolTeacher') { 
                        res.render('teacher/teacher_menu_230328', { ref:thisTeach[0].ref, user:thisTeach[0].firstName, game:thisTeach[0].gameCode, teacherName:thisTeach[0].teacherName, maxStudentGroup:thisTeach[0].maxStudentGroup });
                    }
...
////////////////////////////////////////////////////////////////////////////////
app.get('/memcget/:key', (req, res) => { memC.get(req.params.key, (err, data) => { res.send( 'The retrieved GET value is: '+data ); }); });

app.get('/game_data/:gameCode', (req, res) => {
    memC.get(req.params.gameCode+'_game_status', (err, game_status) => { // res.send( game_status );
        memC.get(req.params.gameCode+'_user_results', (err, user_results) => { // res.send( user_results );
            memC.get(req.params.gameCode+'_user_ranking', (err, user_ranking) => { 
                memC.get(req.params.gameCode+'_linked_users', (err, linked_users) => { 
                    res.send( '<hr>'+game_status+'<hr>'+linked_users+'<hr>'+user_results+'<hr>'+user_ranking+'<hr>' );
                });
            });
        });
    }); 
